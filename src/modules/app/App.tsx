...
import {
  ...
  buildConceptPrompt,
  buildFeedbackPrompt,
  buildPropositionExercisePrompt,
} from "./services/promptBuilder";
import { parseExerciseJson, parseFeedbackJson } from "./services/validators";
import { requestConcept, requestExercise, requestFeedback, listGroqModels } from "./services/groqService";
...
export const App = () => {
  ...
  const handleAttemptSubmit = (topicId: string, themeId: string, content: string) => {
    ...
    if (attempt.cycles >= MAX_CYCLES) {
      showToast("Este intento alcanzó el límite de ciclos.", "error");
      return;
    }
  ...
  const handleGenerateExercise = async (attempt: Attempt, feedback: AttemptFeedback, type: "analitico" | "proposicion") => {
    ...
    if (attempt.cycles >= MAX_CYCLES) {
      showToast("Se alcanzó el límite de ciclos para este intento.", "error");
      return;
    }
    ...
    try {
      const prompt =
        type === "analitico"
          ? buildAnalyticalExercisePrompt({ feedback, conceptText, attempt })
          : buildPropositionExercisePrompt({ feedback, conceptText, attempt });
      ...
      replaceRoute(
        type === "analitico"
          ? { name: "exercise-analytical", attemptId: attempt.attemptId, exercise: payload }
          : { name: "exercise-proposition", attemptId: attempt.attemptId, exercise: payload }
      );
    } catch (error) {
      showToast(error instanceof Error ? error.message : String(error), "error");
    }
  ...
  const handleExportTopic = (topicId: string) => {
    ...
    if (!data) {
      showToast("No se encontró información para exportar.", "error");
      return;
    }
    ...
    showToast("Exportación lista en tus descargas.");
  };
